...
- **V1.1 Multi-user Runtime Contract**：UserContext / Conversation / Message / RunBinding 数据合同。CreateRunRequest 新增可选 user_id / tenant_id / conversation_id / message_id 字段（写入 Run.metadata）。20 条 contract + API 测试。不修改 runtime 行为。详见 [Multi-user Runtime Contract](docs/multi-user-runtime-contract.md)。
- **V1.2 Message / Conversation API**：实现 Conversation/Message CRUD API（6 个 endpoint）和 conversation-triggered run。新增 ConversationRecord/MessageRecord 表（自动创建，无迁移）。Assistant message 在 run completed 且有 output 时回写。旧 `/api/runs` 完全不变。21 条新测试。详见 [Conversation Message API](docs/conversation-message-api.md)。
- **V1.3 Tenant Isolation**：Conversation/Message API 强制 tenant_id。缺失→400，不匹配→404。list conversations 必须 tenant-scoped。POST message/run 校验 user_id 一致性。旧 `/api/runs` 完全不变。详见 [Tenant Isolation](docs/tenant-isolation.md)。
- **V1.4 RAG Tenant Filter**：RAG ingestion/retrieval 支持可选 tenant_id。存储在已有 metadata JSON 列（无数据库迁移）。Keyword / Vector / Hybrid 三种检索模式均支持 tenant 过滤。向后兼容旧无 tenant 文档。详见 [RAG Tenant Filter](docs/rag-tenant-filter.md)。
- **V1.5 Document Cleaning Pipeline**：离线文档清洗流水线。支持 txt / md / pdf / docx / csv / xlsx。类型感知的解析器、清洗器、预览 chunker。Plan A 文档级入库——RAG chunker 负责最终 chunking。metadata 保留 tenant_id / document_key / source_hash / document_version / status。详见 [Ingestion Pipeline](docs/ingestion-pipeline.md)。
- **V1.6 Tool Permission / Ownership Guard**：ToolOwnershipGuard 验证 tenant/user/resource ownership。ToolOwnershipRequirement 加入 ToolDefinition（None=legacy）。guard 集成到 ToolExecutionPipeline。deny 阻止 handler 执行，记录 ToolOwnershipDenied。详见 [Tool Permission Guard](docs/tool-permission-guard.md)。
- **V1.7 Concurrency / Idempotency Contract**：scoped idempotency_key（tenant+user+conversation+action）和 sequence_index 校验。in-memory IdempotencyGuard。message/run 重复 key 返回已有 resource。legacy 兼容。详见 [Concurrency Idempotency Contract](docs/concurrency-idempotency-contract.md)。
- **V1.8 Async Job Queue / Worker Runtime**：DB 后台异步任务队列。Job 状态机（queued/running/succeeded/failed/canceled）。幂等入队（scoped key）。WorkerRuntime.claim_next + handler dispatch。Retry/max_attempts。API create/list/get/cancel。CLI run_worker_once.py。不做 Redis/daemon/business handlers。详见 [Async Job Worker Runtime](docs/async-job-worker-runtime.md)。
...
下一阶段规划：V0.9.6 CLI Docs Consolidation。

### V0.8 能力总结

当前支持：
- Policy / Guardrail / Rule / Condition 合同及 JSON Schema
- PolicyValidator 结构校验及 validation eval runner（22 eval cases）
...
- Provider / RAG guardrail dry-run helpers（helper-level，runtime wiring deferred）

当前不支持：
- **Enforcement** — 不拦截请求，不阻止运行
- **Blocking** — block action 仅作为 decision 记录，不影响实际执行
- **Human review routing** — require_review 仅记录，不被路由
- **Modifying output** — 不修改 tool / provider / RAG result
- **Changing run status** — dry-run hook 不改变 run.status

> 当前阶段 V1.8 为 Async Job Queue / Worker Runtime。DB 后台
> 异步任务队列。不做 Redis / daemon / business handlers。
