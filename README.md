...
下一阶段规划：V0.8.x Guardrail Runtime Integration — Input Guardrail Dry-Run Hook 已完成（V0.8.6），后续为 V0.8.7 Tool Guardrail Dry-Run Hook。

> 当前阶段 V0.8.6 为 Input Guardrail Dry-Run Hook。hook 在 RunStore._create_run
> 中构造 input-scope EvaluationContext，调用 PolicyDryRunEvaluator，
> 记录 guardrail.dry_run.completed event。纯 dry-run，不拦截请求，不 enforcement。
